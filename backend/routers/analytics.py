...
router = APIRouter(tags=["analytics"])  # Removed prefix - handled in main.py


@router.post("/analytics/{post_id}", response_model=schemas.AnalyticsResponse)
def create_analytics(post_id: int, analytics: schemas.AnalyticsCreate, db: Session = Depends(get_db)):
    """Create analytics for a post"""
    db_analytics = Analytics(**analytics.dict(), post_id=post_id)
    db.add(db_analytics)
    db.commit()
...
@router.get("/analytics/post/{post_id}", response_model=schemas.AnalyticsResponse)
...
@router.put("/analytics/post/{post_id}", response_model=schemas.AnalyticsResponse)
